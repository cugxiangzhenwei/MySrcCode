...
CProgressWnd *CProgressWnd::m_pCurProWnd = NULL;  
...
{  
	...
	if (pDC) {  
		...
		CString str = _T("M");  
		for (int i = 0; i < nNumTextLines-1; i++) str += _T("/nM");  
...
int CProgressWnd::SagaCallback(int MessageID, long Param_1, long Param_2)  
{  
	switch( MessageID )  
	{  
	case 1://CALLBACK_PROGRESSBAR_SET_POSITION:  
		if (m_pCurProWnd)  
		{  
			m_pCurProWnd->SetPos((double)Param_2 > 0 ? (int)(100.0 * (double)Param_1 / (double)Param_2) : 100);  
		}  
		break;  
	case 0://CALLBACK_PROGRESS_CHECK:  
		if (m_pCurProWnd)  
		{  
			m_pCurProWnd->PeekAndPump();  
		}  
...
void CProgressWnd::SetCurrentWnd(CProgressWnd *pWnd)  
{  
	m_pCurProWnd = pWnd;  
	m_pCurProWnd->SetRange(0, 100);  
}  

int CProgressWnd::GDALCallBack(double dfComplete, const char *pszMessage, void *pData)  
{  
	if (m_pCurProWnd)  
	{  
		m_pCurProWnd->SetPos((int)(100*dfComplete));  
	}  
	return TRUE;  
}  