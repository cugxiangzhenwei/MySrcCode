...
unsigned  long   WINAPI  ThreadRecvUdpMsg(PVOID   pvoid)
{
	...
	while (1)
	{
		ZeroMemory(szRecv,100);
		//等待接受发送到该主机的消息
		int iRecvStrLen = recvfrom(sockSrv,szRecv,100,0,(sockaddr*)&addClient,&iLen);
		if (iRecvStrLen < 1)
			continue;

		const char * pszFriendIp = inet_ntoa(addClient.sin_addr);

		strcpy(msg.szIp,pszFriendIp);
		strcpy(msg.szNewMsg,szRecv);
...
LRESULT MainFrame::HandleCustomMessage(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	if (uMsg == WM_USER_RECEIVED_NEW_MSG)
	{
		...
		NEW_MSG msg;
		strcpy(msg.szIp,pMsg->szIp);
		strcpy(msg.szNewMsg,pMsg->szNewMsg);
		ReceivedNewMsg(msg);
	}
	return 0;
}
...
{
	tString strMusic = GetExeDir() + "\\Music\\msg.wav";
...
void SendMsgToFriend(const char *pszIp,const char *pszMsg)
{
	...
	sendto(sockClient,pszMsg,strlen(pszMsg)+1,0,(sockaddr*)&addrSrv,sizeof(SOCKADDR));
}
