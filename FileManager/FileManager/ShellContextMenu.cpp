...
void CShellContextMenu::SetObjects(CStringArray &strArray)
{
	...
	MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, strArray[0].GetBuffer(1000), -1, olePath, strArray[0].GetLength () + 1);	
	...
	for (int i = 0; i < nItems; i++)
	{
		...
		MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, strArray[i].GetBuffer(0), -1, olePath, strArray[i].GetLength() + 1);	
		psfDesktop->ParseDisplayName (NULL, 0, olePath, NULL, &pidl, NULL);
		free (olePath);
#else
