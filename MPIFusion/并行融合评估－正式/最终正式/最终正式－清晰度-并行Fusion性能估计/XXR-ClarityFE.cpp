...
//////////////////////                 下面是并行计算清晰度G                            %
///////////////////////////////////////////////////////////////////////////////////// 
/*bool ComputedClarityP(unsigned char *&pColor, double *&clarity, int height,int width)
{
    double *temp,*Fr,*Fg,*Fb;
	int length;
	temp=(double*)malloc(sizeof(double)*3);
...
bool ComputedClarityP(unsigned char *&pColor, double *&clarity, int length,int width)
{
    double *temp,*Fr,*Fg,*Fb;
	//int length;
	int height; 
 ...
 for(int i=1;i<height;i++)
   for(int j=1;j<width;j++)
   {
	   ...
	   SR=sqrt((r1*r1+r2*r2)/2);
	   SG=sqrt((g1*g1+g2*g2)/2);
	   SB=sqrt((b1*b1+b2*b2)/2);
