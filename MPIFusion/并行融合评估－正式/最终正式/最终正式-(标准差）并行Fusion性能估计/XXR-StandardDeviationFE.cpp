...
//bool ComputedStandardCha(unsigned char *&pColor, double meanRGB[3], double *&StandardCha,int height,int width)
bool ComputedStandardCha(unsigned char *&pColor, double meanRGB[3], double *&StandardCha,int length)
...
meanB=meanB/length; meanG=meanG/length; meanR=meanR/length;*/
	meanB=meanRGB[0]; meanG=meanRGB[1]; meanR=meanRGB[2];
...
double fenmu_up_B=0;  
  ...
  // for(int j=0;j<width;j++)
	...
	for(int i=0;i<length;i++)
   {
	 ...
	 /*  fenmu_up_R=fenmu_up_R+(r1-meanB)*(r1-meanB);
	   ...
	   fenmu_up_R=fenmu_up_R+(r1-meanR)*(r1-meanR);
       fenmu_up_G=fenmu_up_G+(g1-meanG)*(g1-meanG);
	   fenmu_up_B=fenmu_up_B+(b1- meanB)*(b1-meanB); 
 ...
 /*  StandardCha[0]= sqrt(fenmu_up_R/(length-1));
   ...
   StandardCha[0]= fenmu_up_R;
   StandardCha[1]= fenmu_up_G;
   StandardCha[2]= fenmu_up_B;
...
double *StandardCha;
ComputedStandardCha(colorDataF, meanFRGB,StandardCha, length);

	if (myid==0)
	{	
		double *StandardCha2;
		StandardCha2=(double*)malloc(sizeof(double)*3*numprocs);

		for (int i=0;i<numprocs*3;i++)		
			StandardCha2[i]=0;
		for (int i=0;i<3;i++)		
			StandardCha2[i]=StandardCha[i];

	//	StandardCha2=StandardCha;
	//	MPI_Send(&StandardCha,3,MPI_DOUBLE,0,50,MPI_COMM_WORLD);
	//	MPI_Recv(StandardCha2,3,MPI_DOUBLE,0,50,MPI_COMM_WORLD,&s);
	  
      for (int i=1;i<numprocs;i++)
		{
			  MPI_Recv(StandardCha2+3*i,3,MPI_DOUBLE,i,61,MPI_COMM_WORLD,&s);

		}
	  
	  double *StandardChaLast;
		StandardChaLast=(double*)malloc(sizeof(double)*3);
	 ...
	 {
		//for (int k=0;k<3;k++)
	    // StandardChaLast[k]=StandardChaLast[k]+StandardCha2[i++];
        StandardChaLast[0]=StandardChaLast[0]+StandardCha2[i++];
        StandardChaLast[1]=StandardChaLast[1]+StandardCha2[i++];
        StandardChaLast[2]=StandardChaLast[2]+StandardCha2[i++];

	 }
	 for (int i=0;i<3;i++)
		{
		 StandardChaLast[i]=sqrt(StandardChaLast[i]/colorLength);
		}

  printf("StandardChaLast[0]=%.3f,StandardChaLast[1]=%.3f,StandardChaLast[2]=%.3f\n\n",StandardChaLast[0],StandardChaLast[1],StandardChaLast[2]);
   
	} 

	else
	{ 	
	MPI_Send(StandardCha,3,MPI_DOUBLE,0,61,MPI_COMM_WORLD);
	}
