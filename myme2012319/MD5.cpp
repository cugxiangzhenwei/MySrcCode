...
static void byteReverse(unsigned char* buf, unsigned int longs)
{
    ...
    do
    {
        t = (unsigned int) ((unsigned int) buf[3] << 8 | buf[2]) <<
            16 |
...
MD5::MD5() : m_private(NULL)
{
    //#[ operation MD5() 
    //#]
}

MD5::MD5(const MD5& original) : m_private(NULL)
{
...
MD5::MD5(const void* buf, int len) : m_private(NULL)
{
    //#[ operation MD5(const void *,ACE_INT32) 
    Update(buf, len);

    //#]
}

MD5::MD5(const std::string& str) : m_private(NULL)
{
...
const std::string& MD5::HexDigest()
{
    //#[ operation HexDigest() 
    Finalize();
    return m_hex;
