...
应用程序向导已为您创建了此 OverlayIcon 项目，作为编写动态链接库 (DLL) 
的起点。
...
OverlayIcon.idl
    ...
    此文件将由 MIDL 编译器处理以生成：
        ...
        类型库				(OverlayIcon.tlb)
        封送处理代码			(OverlayIcon_p.c 和 
                                           dlldata.c）

OverlayIcon.h
    此文件包含 OverlayIcon.idl 中定义的项目的 C++ 接口定义
...
StdAfx.h，StdAfx.cpp
    这些文件用于生成名为 OverlayIcon.pch 的预编译头 (PCH) 文件和
...
OverlayIconps.vcxproj
    ...
    文件。此进程生成 dlldata.c、OverlayIcon_i.c 和 
    OverlayIcon_p.c，这些文件是生成代理/存根 (stub) DLL 所必需的。
