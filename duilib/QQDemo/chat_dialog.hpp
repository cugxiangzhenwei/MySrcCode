...
public:
	...
	ChatDialog(MainFrame *pMain,const tString& bgimage, DWORD bkcolor, const FriendListItemInfo& myselft_info, const FriendListItemInfo& friend_info);
	~ChatDialog();
...
public:
    ...
    void SendMsg();
	void ReceivedMsg(tString strMsg);
	void ShakeWindow(bool bBtnClicked); // ¶¶¶¯´°¿Ú
	void SelectSendFile();
	void SelectSaveFile(tString strFile);
	void SetProgressRange(int iMin,int iMax);
	void SetProgressMsg(tString strMsg);
	void SetProgressFileName(tString strFileName);
	void UpdateProgressValue(int iValue);
	void HideProgressAndLable();
