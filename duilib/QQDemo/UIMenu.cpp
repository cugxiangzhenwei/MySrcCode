...
static ContextMenuParam g_menuParam;
...
BOOL CMenuWnd::Receive(ContextMenuParam param)
{
	...
	case 1:
		{
			if (param.strMunuName != "")
			{
				g_menuParam = param;
				::PostMessage(m_hParent, WM_USER_MENU_MSG, 0,(LPARAM)&g_menuParam); //发送菜单消息到主窗口，主窗口进行处理
			}
			Close();
			break;
		}
